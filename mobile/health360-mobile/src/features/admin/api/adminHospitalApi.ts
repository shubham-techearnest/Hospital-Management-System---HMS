...
export const HOSPITAL_STATUSES = ['ACTIVE', 'INACTIVE', 'SUSPENDED'] as const;
...
export async function updateAdminPlanLimits(
  ...
  const { data } = await apiClient.patch<ApiEnvelope<SubscriptionPlan>>(`/admin/plans/${planId}/limits`, {
    limits,
  });
  return data.data!;
}
