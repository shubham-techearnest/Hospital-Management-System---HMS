...
import com.health360.laboratory.application.service.LabPatientJourneyService;
...
public class LabController {
    ...
    private final LabPatientJourneyService patientJourneyService;
    ...
    public ResponseEntity<ApiResponse<List<PatientLabOrderResponse>>> listMyLabOrders(
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(ApiResponse.ok(patientJourneyService.listMyLabOrders(principal)));
    }

    @PostMapping("/me/orders/{clinicalOrderItemId}/book-hospital")
    @PreAuthorize("hasAuthority('clinical:encounter:read')")
    public ResponseEntity<ApiResponse<PatientLabOrderResponse>> bookHospitalLab(
            @AuthenticationPrincipal UserPrincipal principal,
