...
public class FacilityAccessService {
    ...
    public void assertCanRead(UserPrincipal principal) {
        if (!principal.hasPermission("facility:read")) {
            throw forbidden();
        }
    }

    public void assertCanWrite(UserPrincipal principal) {
        if (!principal.hasPermission("facility:write")) {
            throw forbidden();
        }
    }

    public void assertCanComplete(UserPrincipal principal) {
        if (!principal.hasPermission("facility:complete")
                && !principal.hasPermission("facility:write")) {
            throw forbidden();
        }
    }
    ...
    public void assertModuleEnabled(UserPrincipal principal, UUID hospitalId, UUID branchId) {
        ...
        featureAccessService.assertHasFeature(
                ...
                PlanFeatureKeys.FEATURE_FACILITY_OPS,
                "Facility operations are not included in this hospital's subscription plan");
    }
