...
export function useDoctorBookingLocations(doctorId: string, enabled = true) {
  return useQuery({
    ...
    enabled: enabled && isValidUuid(doctorId),
  });
}
...
export function useDoctorAvailability(
  ...
  return useQuery({
    ...
    enabled: enabled && isValidUuid(doctorId) && Boolean(hospitalId && branchId),
    staleTime: 30_000,
  });
}
...
export function useMyAppointments(filter: AppointmentFilter = 'all') {
  return useQuery({
