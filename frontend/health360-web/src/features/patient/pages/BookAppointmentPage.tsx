      ...
      {activeStep === 0 ? (
        <Box>
          <Typography variant="subtitle1" sx={{ mb: 2 }}>
            Where will you meet {locations[0]?.doctorName ?? 'the doctor'}?
          </Typography>
                      ...
                      label={
                        <Box>
                          <Typography fontWeight={600}>
                            {location.doctorName
                              ? `${location.doctorName} at ${location.hospitalName}`
                          ...
                          {location.opdHours && location.opdHours.length > 0 ? (
                            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                              OPD: {location.opdHours.slice(0, 4).join(' · ')}
                              {location.opdHours.length > 4 ? ' …' : ''}
                            </Typography>
                          ) : (
                            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                              Pick a date next to see open slots for this location.
                            </Typography>
                          )}
                        </Box>
                      }
          ...
          <Card variant="outlined">
            <CardContent>
              ...
              {selectedLocation.doctorName ? (
                <Typography sx={{ mt: 1 }}><strong>Doctor:</strong> {selectedLocation.doctorName}</Typography>
              ) : null}
              <Typography sx={{ mt: selectedLocation.doctorName ? 0 : 1 }}>
                <strong>Hospital:</strong> {selectedLocation.hospitalName}
              </Typography>
